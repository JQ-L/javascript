...
##  开发模式(develope)和发布模式(production)

开发模式正常使用的是npm下再的依赖 运行的话执行npm run serve 

发布模式使用的是外链资源 他是被build进行打包成一个整体的dist文件 里面包含js css 等一些文件 他的依赖需要联网才能使用 并且需要通过node的静态资源进行访问 

开发模式和发布模式 是有两个入口的main文件 打包的时候 为了让发布模式的依赖项的文件尽可能减小 将依赖文件全部转换成url联网访问 这样可以不让第三方的css和js一起打包进入dist文件中 
